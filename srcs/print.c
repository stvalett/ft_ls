...
/*   Updated: 2017/02/05 11:39:22 by stvalett         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_link_bis(t_dir *dir, t_opt *opt)
{
    ...
    {
        ft_putstr(dir->name);
        if ((path_size = readlink(dir->name, buf, MAX_PATH)) < 0)
            return ;
        buf[path_size] = '\0';
        ft_putstr(" -> ");
        ft_putendl(dir->name);
...
void		print_all(t_dir *current, int len, t_opt *opt, int flag)
{
    if (current->stat_current == 1 || current->stat_handle == 1)
    {
        ...
        if (!opt->o_l && !opt->o_u && !opt->o_g && !opt->o_1)
            display_simple(current, len, opt);
        else if (!opt->o_up_a || opt->o_a)
